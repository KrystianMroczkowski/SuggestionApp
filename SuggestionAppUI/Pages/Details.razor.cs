	...
	private async Task CompleteSetStatus()
	{
		switch (settingStatus)
		{
			case "completed":
				...
				suggestion.OwnerNotes = $"You are right, this is an important topic for developeres. We created a resouce about it here: <a href='{urlText}' target='_blank'>{urlText}</a>";
				break;
			...
			case "dismissed":
				suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
				suggestion.OwnerNotes = "Sometimes a good idea doesn't fit within our scope nad vision. This is one of those ideas.";
				break;
